...
function loadUser() {
  try {
    ...
    const theme = localStorage.getItem("att_theme");
    if(theme === "light") {
...
const LOGO = `<img src="logo.png" class="logo" alt="Logo" />`;
const LOGO_LG = `<img src="logo.png" class="logo-lg" alt="Logo" />`;
const THEME_BTN = `<button class="theme-toggle" id="themeBtn">${state.darkMode ? '☀️' : '🌙'}</button>`;
  ...
  <div style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:2rem;">
    <div style="width:100%;max-width:380px;">
      <div style="text-align:center;margin-bottom:2rem;">
        ${LOGO_LG}
        <div style="font-size:26px;font-weight:800;margin-bottom:4px;color:var(--text);">Attendance Portal</div>
        <div style="font-size:13px;color:var(--muted);">Sign in to your account</div>
      </div>
      ${alertHtml()}
...
function topbar(title, backBtn = null) {
  return `
  <div class="topbar">
    <div style="display:flex;align-items:center;gap:12px;">
      ${LOGO}
      ${title ? `<span style="font-weight:700;font-size:15px;color:var(--text);">${title}</span>` : ""}
    </div>
    <div class="row" style="gap:8px;">
      ${THEME_BTN}
      ${backBtn ? `<button class="btn btn-sm" id="${backBtn.id}">${backBtn.label}</button>` : ""}
    </div>
  </div>`;
}
      ...
      <div class="row" style="justify-content:space-between;margin-bottom:1rem;">
        <div>
          ...
          <div style="font-size:12px;color:var(--muted);margin-top:2px;">
            ...
            <span class="badge badge-${roleColor}" style="margin-left:6px;">${roleLabel}</span>
          </div>
        </div>
        <div class="row" style="gap:8px;">
            ...
            <div class="card session-card">
              <div class="row" style="justify-content:space-between;margin-bottom:6px;">
                ...
                <div class="row" style="gap:6px;">
                  ...
                  <button class="btn btn-sm" style="background:#ef444420;color:#ef4444;border-color:#ef444440;" data-delete="${s.code}">🗑</button>
                </div>
              </div>
              ...
              <div class="session-link">
                Share code: <strong style="color:#60a5fa;font-size:15px;letter-spacing:2px;">${s.code}</strong>
              </div>
            </div>`;
          }).join("")}
    </div>
...
function renderCreateSession() {
  ...
  return `
  <div>
    ${topbar("Create Session", { id:"backBtn", label:"← Back" })}
    <div class="main">
      ${alertHtml()}
      <div class="card col" style="gap:1rem;">
        <div class="col">
        ...
        <div style="font-size:14px;font-weight:700;color:var(--text);">
          Student list
          <span style="font-weight:400;color:var(--muted);font-size:12px;">(${ns.students.length} added)</span>
        </div>
        <div class="row">
        ...
        <div class="scroll-area">
          ${ns.students.length === 0
            ...
            <div class="row" style="padding:7px 10px;border-radius:8px;border:1px solid var(--border);margin-bottom:6px;justify-content:space-between;">
              <span style="font-size:13px;color:var(--text);">${i+1}. ${st}</span>
...
function renderEditSession() {
  ...
  return `
  <div>
    ${topbar(`Edit — ${s.title}`, { id:"backBtn", label:"← Back" })}
    <div class="main">
      ${alertHtml()}
      <div class="card col" style="gap:1rem;">
        <div style="font-size:14px;font-weight:700;color:var(--text);">
          Toggle attendance
          <span style="font-weight:400;color:var(--muted);font-size:12px;">(tap to mark present/absent)</span>
        </div>
        <div class="scroll-area">
          ${students.map((st,i) => `
          <div class="student-row ${st.present?'present':''}" data-mark="${i}">
            <span style="font-size:14px;color:var(--text);">${i+1}. ${st.name}</span>
            <span class="badge badge-${st.present?'green':'amber'}">${st.present?'Present':'Absent'}</span>
          </div>`).join("")}
        </div>
        <div class="divider"></div>
        <div style="font-size:14px;font-weight:700;color:var(--text);">Add new student</div>
        <div class="row">
...
function renderStudentHome() {
  ...
  <div>
    ...
    <div class="main">
      <div class="row" style="justify-content:space-between;margin-bottom:1rem;">
        <div>
          <div style="font-size:18px;font-weight:700;color:var(--text);">Welcome, ${u.name}</div>
      ...
      <div class="card col" style="gap:0.75rem;margin-bottom:1.25rem;">
        <div style="font-size:14px;font-weight:700;color:var(--text);">Enter Session Code</div>
        <div style="font-size:12px;color:var(--muted);">Ask your lecturer or course rep for the 6-digit code.</div>
        <div class="row">
      ...
      <div style="font-size:14px;font-weight:700;margin-bottom:8px;color:var(--text);">
        Active Sessions
        <span style="font-weight:400;color:var(--muted);font-size:12px;">(live)</span>
      </div>
      ${activeSessions.length === 0
        ...
        <div class="card session-card" style="cursor:pointer;" data-join="${s.code}">
          <div class="row" style="justify-content:space-between;">
            <div>
              <div style="font-weight:700;color:var(--text);">${s.title}</div>
...
function renderStudentCheckin() {
  ...
  if(!s) return `<div style="padding:2rem;text-align:center;color:var(--muted);">Session not found.</div>`;
  const students = Object.values(s.students || {});
  ...
  return `
  <div>
    ${topbar(s.title, { id:"backToHomeBtn", label:"← Back" })}
    <div class="main">
      <div style="font-size:13px;color:var(--muted);margin-bottom:1rem;">
        Select your name, scroll down, then tap <strong style="color:var(--text);">Mark Present</strong>.
      </div>
      ${alertHtml()}
      <div style="font-size:13px;font-weight:700;margin-bottom:8px;color:var(--text);">
        Select your name
        <span style="font-weight:400;color:var(--muted);">(${students.length} students)</span>
      </div>
      ...
      <div style="background:var(--surface);border-radius:8px;padding:10px;font-size:12px;color:var(--muted);margin-bottom:1rem;border:1px solid var(--border);">
        📍 Required GPS radius: <strong style="color:var(--text);">${s.radius}m</strong> from classroom.
      </div>
...
function doMarkPresent() {
  ...
  navigator.geolocation.getCurrentPosition(pos => {
    ...
    if(dist <= s.radius) {
      ...
      state.locationStatus = { ok: false, msg: `❌ You are ${Math.round(dist)}m away. Must be within ${s.radius}m. (±${accuracy}m accuracy)` };
      render();
    }
  }, err => {
